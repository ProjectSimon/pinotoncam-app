...
void DngEncoder::setup_encoder(libcamera::StreamConfiguration const &cfg, libcamera::StreamConfiguration const &lo_cfg, CompletedRequest::ControlList const &metadata)
{
    ...
    if (bl)
    {
        ...
        LOG(4, "WARNING: no black level found, using default");
    ...
    if (ccm)
    {
        ...
        LOG(4, "WARNING: no CCM metadata found");
    ...
    LOG(1, "Max Frames in Buffer: " << max_buffer_frames);

    encoder_initialized_ = true;
    LOG(2, "DngEncoder is setup!");
}
              ...
              ControlList const &metadata, uint64_t fn)
{
    ...
    if (exp)
        ...
        LOG(4, "WARNING: default to exposure time of " << exp_time << "us");
    exp_time /= 1e6;
    ...
    if (ag)
        ...
        LOG(4, "WARNING: default to ISO value of " << iso);
    ...
    if (!memBuf.buffer) {
        LOG(4, "Failed to allocate memory\n");
        exit(1);
    }
    memBuf.offset = 0;
    memBuf.usedSize = 0;
    ...
    LOG(1, "thrd: " << thread_num << " Writing DNG " << fn);
    try
    {
        ...
        LOG(1, "thrd: " << thread_num << " Writing DNG thumbnail " << fn);
        ...
        LOG(1, "thrd: " << thread_num << " Writing DNG thumbnail end " << fn);
        ...
        LOG(1, "thrd: " << thread_num << " Writing DNG main image " << fn);
        ...
        LOG(1, "thrd: " << thread_num << " Writing DNG main dict " << fn);
        TIFFWriteDirectory(tif);

        LOG(1, "thrd: " << thread_num << " Writing DNG EXIF "<< fn);
        
        TIFFCreateEXIFDirectory(tif);

        LOG(1, "thrd: " << thread_num << " Writing DNG TIFFCreateEXIFDirectory " << fn);
        char time_str[32];
        ...
        LOG(1, "thrd: " << thread_num << " Writing DNG TIFFClose " << fn);
        return memBuf.usedSize;
    }
...
void DngEncoder::encodeThread(int num)
{
    ...
    while (true)
    {
        ...
        {   
            ...
            LOG(2, "Thread[" << num << "] " <<  encode_item.index << " Time taken for the encode: " << duration << " milliseconds, disk buffer count: " << amount << " Size: " << tiff_size);
        }
...
void DngEncoder::diskThread(int num)
{
    ...
    while (true)
    {
        ...
        auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end_time - start_time).count();
        LOG(2, "Thread[" << num << "] "<< disk_item.index << " Time taken for the disk io: " << duration <<" milliseconds");
